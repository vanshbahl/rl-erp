...
from app.models.enums import UserRole, ProductType
...
def validate_bom_constraints(db: Session, parent_product_id: int, items: list) -> None:
    # 1. Verify parent product exists and is type FINISHED_GOOD or SEMI_FINISHED
    ...
    # 2. Check for duplicate components in the request
...
def handle_active_bom_deactivation(db: Session, product_id: int, exclude_bom_id: int | None = None) -> None:
...
def create_bom(
    ...
    validate_bom_constraints(db, bom_data.product_id, bom_data.items)

    if bom_data.is_active:
        handle_active_bom_deactivation(db, bom_data.product_id)
...
def update_bom(
    ...
    if bom_data.is_active is not None:
        if bom_data.is_active and not bom.is_active:
            handle_active_bom_deactivation(db, bom.product_id, exclude_bom_id=bom.id)
        bom.is_active = bom_data.is_active

    if bom_data.items is not None:
        # Run validations on items
        validate_bom_constraints(db, bom.product_id, bom_data.items)
