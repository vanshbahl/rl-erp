...
- **`inventory_transactions`**: The immutable ledger. Every quantity mutation to the `inventory` table writes a row here. Uses `transaction_type` enum values: `SALE`, `PURCHASE_RECEIPT`, `ADJUSTMENT`, `PRODUCTION_CONSUMPTION`, `PRODUCTION_OUTPUT`, `REVERSAL`, `ORDER_DISPATCH`, and `ORDER_CANCEL`. Manual quantity changes through `PUT /inventory/{product_id}` write an `ADJUSTMENT` row atomically with the inventory update; minimum-stock-only changes do not create quantity ledger entries. Dispatch and dispatched-order cancellation write `ORDER_DISPATCH` and `ORDER_CANCEL` rows respectively in the same transaction as inventory mutation.
