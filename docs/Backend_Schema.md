...
- **`inventory_transactions`**: The immutable ledger. Every quantity mutation to the `inventory` table writes a row here. Uses `transaction_type` enum values: `SALE`, `PURCHASE_RECEIPT`, `ADJUSTMENT`, `PRODUCTION_CONSUMPTION`, `PRODUCTION_OUTPUT`, `REVERSAL`. Manual quantity changes through `PUT /inventory/{product_id}` write an `ADJUSTMENT` row atomically with the inventory update; minimum-stock-only changes do not create quantity ledger entries. Additionally, the order dispatch flow writes `ORDER_DISPATCH` and `ORDER_CANCEL` as raw string values — these are NOT in the `InventoryTransactionType` enum and represent a known inconsistency.
